...
class ImageEditorApp(QMainWindow):
    ...
    def _build_duplicates_tab(self, tab: QWidget):
        ...
        self.dup_status_lbl = QLabel("Load a folder in Photos/Live tabs, then press 'Scan for duplicates'.")
        ...
        self.dup_scan_button.setToolTip("Scan the currently loaded folder for duplicate images or videos.")
        ...
        def _run_duplicate_scan():
            ...
            if mode == "Images":
                ...
                if not self.current_folder or not self.mov_files:
                    QMessageBox.information(
                        self,
                        "Duplicates",
                        "No videos loaded.\n\nLoad a folder with videos in the Live tab first.",
                    )
                    return
            ...
            if mode == "Images":
                ...
                worker = VideoDuplicateWorker(self.mov_files)

            def on_progress(pct: int):
                self.dup_status_lbl.setText(f"Scanning... {pct}%")
            ...
            def on_finished(groups, msg: str):
                ...
                if not self.duplicate_groups:
                    # msg already says "No duplicates...", just leave list empty
                    return
                ...
                for idx, group in enumerate(self.duplicate_groups):
                    ...
                    # Generic label: can be images or videos
    ...
    def _video_path_to_pixmap(self, path: str, thumb_size: int) -> Optional[QPixmap]:
        """
        Grab a single representative frame from a video and convert it into
        a QPixmap suitable for thumbnails. Returns None on failure.
        """
        if _CV2 is None:
            return None
        try:
    ...
    def _on_duplicate_thumbnail_clicked(self, path: str, multi_select: bool = False) -> None:
        """
        Handle click on a duplicate thumbnail.

        - Always navigates to the image (same behavior as before).
        - If multi_select is True (Ctrl/Cmd held), toggles the selection of this path.
        - If multi_select is False, clears previous selection and selects only this path.
        """
        # Navigate to the image in the Photos tab (existing behavior)
        try:
            idx = self.image_files.index(path)
        except ValueError:
            # Not found in current image list - nothing to navigate to
            idx = -1
        ...
        if multi_select:
            ...
            if path in self._selected_duplicate_paths:
                ...
                # Keep _selected_duplicate_path pointing to some remaining selection
                if self._selected_duplicate_path == path:
                    self._selected_duplicate_path = (
                        next(iter(self._selected_duplicate_paths), None)
                        if self._selected_duplicate_paths
                        else None
                    )
            else:
            ...
            # Plain click: make this the only selected thumbnail
    ...
    def load_directory_async(self, folder: Path):
        ...
        # Pass a wrapper or reference to the extraction function, not the bound method if possible
        # but DirScanJob in tasks.py expects a callable
    ...
    def _on_scan_found_image(self, job_id: int, path: str, taken: str):
        ...
        if hasattr(self, "dup_status_lbl"): self.dup_status_lbl.setText("Load a folder in Photos tab, then scan.")
    ...
    def _perform_save(self, save_path: str) -> bool:
        try:
            ...
            # Temporarily reuse PreviewJob logic for edit application or duplicate it here.
            # For simplicity, let's just do manual application since PreviewJob is async and we need sync save.
            # But wait, PreviewJob has an internal _apply_edits method? It's instance method.
            # Let's instantiate a dummy job to reuse the logic.
    ...
    def _on_delete_key(self):
        ...
        - If there are selected duplicate thumbnails in the Duplicates tab
          (self._selected_duplicate_paths), delete ALL of them.
        - Otherwise, delete the current image (self.current_image_index) as before.

        Files are moved into a sibling "deleted" folder next to each file.
        """
        # Determine which paths to delete
        selected_paths = set(getattr(self, "_selected_duplicate_paths", set()))

        if not selected_paths:
            # Fallback: delete the currently shown image
            if not self.image_files or self.current_image_index < 0:
                return
        ...
        if len(selected_paths) == 1:
            path_str = next(iter(selected_paths))
            path = Path(path_str)
            ...
            msg = (
                f"Move {len(selected_paths)} files into their 'deleted' folders?\n\n"
                "Each file will be moved into a 'deleted' subfolder next to it."
            )
        ...
        if errors:
            # We still proceed with successfully moved files
        ...
        # Update image_files, photo_list, and image_meta
        # Collect indices to remove so we can remove them safely from the end
        ...
        for idx in indices_to_remove:
            ...
            if hasattr(self, "photo_list") and self.photo_list is not None:
                item = self.photo_list.takeItem(idx)
                if item is not None:
                    del item
            # Drop metadata
        ...
        # Update duplicate groups (remove deleted paths)
        if self.duplicate_groups:
            new_groups: List[List[DuplicateRecord]] = []
            ...
            if hasattr(self, "dup_groups_list"):
                ...
                for idx_g, group in enumerate(self.duplicate_groups):
                    try:
                        ...
                        folder_name = "n/a"
                    item_text = (
                        f"Group {idx_g + 1}: {len(group)} images (folder: {folder_name})"
                    )
                    self.dup_groups_list.addItem(item_text)
                self.dup_groups_list.blockSignals(False)

            # Reset duplicate thumbnails and selection
            self._clear_duplicate_thumbnails()

            if self.duplicate_groups:
                # Select first remaining group by default
                if hasattr(self, "dup_groups_list"):
                    self.dup_groups_list.setCurrentRow(0)
        ...
        # Clear selection of duplicates (they are gone now)
        ...
        # Maintain current image / viewer state
        if not self.image_files:
            self.current_image_index = -1
            # No images left - clear viewer and controls
            self._clear_photos_state(reset_lists_only=False)
            self.set_controls_state(False)
            return

        # Try to keep a reasonable current image index
        # If original current image was deleted, pick the closest valid index
        try:
            # If there is still some selected path that survived, focus on it
            if self._selected_duplicate_paths:
                # Arbitrarily pick one of the remaining selected
                focus_path = next(iter(self._selected_duplicate_paths))
                new_idx = self.image_files.index(focus_path)
            else:
                # Fallback: clamp old current_image_index
...
class VideoDuplicateWorkerSignals(QObject):
    """
    Signals for VideoDuplicateWorker:
      - progress: int percentage [0..100]
      - finished: (groups: List[List[DuplicateRecord]], msg: str)
    """
...
class VideoDuplicateWorker(QRunnable):
    """
    QRunnable that runs VideoDuplicateFinder.find_video_duplicates on a
    background thread and reports progress / completion via signals.
    """
    ...
    def run(self) -> None:
        ...
        msg = (
            f"Found {len(groups)} duplicate video groups."
            if groups
            else "No duplicate videos found."
        )
